...
defined(WEBOS_INSTALL_WEBOS_APPLICATIONSDIR, var) {
    INSTALL_APPDIR = $$WEBOS_INSTALL_WEBOS_APPLICATIONSDIR/com.photogallery.app.qml
    ...
    base.files = main.qml PhotoDelegate.qml

    icon.path = $$INSTALL_APPDIR
    icon.files = icon.png

    resources.path = $$INSTALL_APPDIR
    resources.files = qml.qrc Photos

    INSTALLS += target appinfo base icon resources
}
